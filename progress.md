...
## §4 Modeling — the debiased head-to-head, and the popularity-exploitation finding (2026-06-04)

`.venv` restored; bridge ran end-to-end on CPU. Confirmation runs (`bridge_full.yaml`, Books→Video_Games thin + Books→Movies rich) under three eval protocols told a progressively more honest story — and the final one reframes the whole result. New config knobs on `bridge.py`: `holdout` (temporal|random), `neg_sampling` (uniform|popularity), `compare_original` (train+eval the original free-embedding EMCDR mapping in the same harness for a true head-to-head).

- **Uniform negatives + temporal holdout:** bridge looked strong — beat the content baseline by +74%/+123% (Books→VG 0.326 vs 0.187; Books→Movies 0.445 vs 0.199), and reversed the old EMCDR's thin-pair loss. But MostPop was suspiciously dominant (~0.47).
- **The debias (popularity-weighted negatives):** drawing eval negatives ∝ popularity neutralizes MostPop's free edge. Result (Recall@10): **MostPop collapsed to ~random (0.080 VG / 0.059 Movies), and so did BOTH learned mappings** — bridge 0.074/0.102, emcdr_original 0.096/0.100, all ≈ random (0.098). **The only method that stays above random is the content cosine `feature_transfer` (0.165 / 0.194).**
- **Conclusion:** under honest (popularity-matched) evaluation, the learned cross-domain methods — new bridge AND original EMCDR alike — were largely **exploiting popularity, not learning taste transfer**. They were trained against *uniform* negatives, so they learned "rank popular items high," which scores great vs random negatives and collapses to random once negatives are popularity-matched. The simple content baseline is the only one transferring genuine *semantic* signal. **Redesign vs original is a wash under fair eval (both ≈ random)** — the mapping architecture wasn't the binding constraint; popularity-encoding embeddings (id_factor + `pop`, trained on uniform negatives) were.
- **Next:** match training to the honest objective — **train with popularity-weighted / hard negatives** (recommenders + bridge), and ablate the `pop` feature / anchor item embeddings on raw MiniLM. Content (`feature_transfer`) is now the benchmark to beat.

Results: `results/bridge__*__bridge_full.json` + `results/BRIDGE_GRID__bridge_full.json` hold the final debiased head-to-head (uniform-negative numbers above are recorded here only, since the JSONs were overwritten by the debiased re-run).
